...
from core.services.management_reports.funds_service import FundsReportService
...
class FundsController:
    def __init__(self, app):
        self.funds_service = FundsReportService()
    ...
    def register_routes(self) -> None:
        ...
        self.app.add_url_rule(
            "/api/v1/rg-fundos", "rg_funds_handler", self.handler, methods=["POST"]
        )
    ...
    def _handle_initial_request(self) -> Tuple[Dict[str, Any], int]:
        """
        Handle the initial Funds report request.
        Returns:
            Tuple containing response data and HTTP status code
        """
        self.logger.log_and_respond(
            "Iniciando requisição de relatório gerencial de Fundos."
        )

        if not self.funds_service.get_funds_report():
            return {
                "error": "Falha ao iniciar a requisição de relatório gerencial de Fundos."
    ...
    def _process_webhook(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
        ...
        csv_data = self.funds_service.process_csv_from_url(csv_url)
        if not csv_data:
            self.logger.logger.info(
                "Não foram encontrados dados para o relatório de Fundos."
            )
            return {
                "message": "Não foram encontrados dados para o relatório de Fundos."
            }, HTTPStatus.NO_CONTENT

        self.logger.logger.info("Relatório de Fundos gerado com sucesso.")
