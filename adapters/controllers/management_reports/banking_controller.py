...
from core.services.management_reports.banking_service import BankingService
...
class BankingController:
    ...
    def register_routes(self) -> None:
        ...
        self.app.add_url_rule(
            "/api/v1/rg-banking", "rg_banking_handler", self.handler, methods=["POST"]
        )
    ...
    def _handle_initial_request(self) -> Tuple[Dict[str, Any], int]:
        """
        Handle the initial Banking report request.
        Returns:
            Tuple containing response data and HTTP status code
        """
        self.logger.log_and_respond(
            "Iniciando requisição de relatório gerencial Banking."
        )

        if not self.banking_service.get_banking_report():
            return {
                "error": "Falha ao iniciar a requisição de relatório gerencial Banking."
    ...
    def _process_webhook(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
        ...
        csv_data = self.banking_service.process_csv_from_url(csv_url)
        if not csv_data:
            self.logger.logger.info(
                "Não foram encontrados dados para o relatório Banking."
            )
            return {
                "message": "Não foram encontrados dados para o relatório Banking."
            }, HTTPStatus.NO_CONTENT

        self.logger.logger.info("Relatório Banking gerado com sucesso.")
