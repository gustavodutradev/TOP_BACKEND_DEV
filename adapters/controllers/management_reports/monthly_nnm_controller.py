...
from core.services.management_reports.monthly_nnm_service import MonthlyNNMService
...
class MonthlyNNMController:
    def __init__(self, app):
        self.monthly_nnm_service = MonthlyNNMService()
        self.app = app
    ...
    def _handle_initial_request(self) -> Tuple[Dict[str, Any], int]:
        """
        Handle the initial NNM report request.
        Returns:
            Tuple containing response data and HTTP status code
        """
        ...
        if not self.monthly_nnm_service.get_monthly_nnm_report():
            return {
                "error": "Falha ao iniciar a requisição de relatório gerencial mensal de NNM."
    ...
    def _process_webhook(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
        ...
        return {"message": "Relatório gerencial mensal de NNM gerado com sucesso."}, HTTPStatus.OK
