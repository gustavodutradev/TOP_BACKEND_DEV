...
from core.services.management_reports.closed_commissions_service import (
    ClosedCommissionsService,
)
...
class ClosedCommissionsController:
    def __init__(self, app):
        self.closed_commissions_service = ClosedCommissionsService()
    ...
    def register_routes(self) -> None:
        ...
        self.app.add_url_rule(
            "/api/v1/rg-monthly-commissions",
            "rg_monthly_commissions_handler",
            self.handler,
            methods=["POST"],
        )
    ...
    def _handle_initial_request(self) -> Tuple[Dict[str, Any], int]:
        ...
        if not self.closed_commissions_service.get_closed_commissions_report():
            return {
    ...
    def _process_webhook(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
        ...
        csv_data = self.closed_commissions_service.process_csv_from_url(csv_url)
        if csv_data is None:
