...
from core.services.management_reports.exchange_service import ExchangeService
...
class ExchangeController:
    ...
    def register_routes(self) -> None:
        ...
        self.app.add_url_rule(
            "/api/v1/rg-exchange", "rg_exchange_handler", self.handler, methods=["POST"]
        )
    ...
    def _handle_initial_request(self) -> Tuple[Dict[str, Any], int]:
        ...
        self.logger.log_and_respond(
            "Iniciando requisição de relatório gerencial de Câmbio"
        )

        if not self.exchange_service.get_exchange_report():
            return {
                "error": "Falha ao iniciar a requisição de relatório gerencial de Câmbio"
    ...
    def _process_webhook(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
        ...
        csv_data = self.exchange_service.process_csv_from_url(csv_url)
        if not csv_data:
            self.logger.logger.info(
                "Não foram encontrados dados para o relatório de Câmbio."
            )
            return {
                "message": "Não foram encontrados dados para o relatório de Câmbio."
            }, HTTPStatus.NO_CONTENT

        self.logger.logger.info("Relatório de Câmbio gerado com sucesso.")
