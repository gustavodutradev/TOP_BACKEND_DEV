...
class RelationshipController:
    ...
    def _handle_initial_request(self) -> Tuple[Dict[str, Any], int]:
        ...
        self.logger.log_and_respond(
            "Iniciando requisição de relação de contas e assessores."
        )

        if not self.relationship_service.get_account_advisors_link():
            return {
                "error": "Falha ao iniciar a requisição de relação de contas e assessores."
    ...
    def _process_webhook(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
        ...
        if not csv_data:
            self.logger.logger.info(
                "Não foram encontrados dados para o relatório de relação de contas e assessores."
            )
            return {
                "message": "Não foram encontrados dados para o relatório de relação de contas e assessores."
            }, HTTPStatus.NO_CONTENT

        self.logger.logger.info(
            "Relatório de relação de contas e assessores gerado com sucesso."
        )
