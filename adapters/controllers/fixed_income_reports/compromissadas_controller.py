...
class CompromissadasController:
    def __init__(self, app):
        self.compromissadas_service = CompromissadasService()
    ...
    def _handle_initial_request(self) -> Tuple[Dict[str, Any], int]:
        ...
        self.logger.log_and_respond(
            "Iniciando requisição de relatório RF de Compromissadas."
        )

        if not self.compromissadas_service.get_compromissadas_report():
            return {
                "error": "Falha ao iniciar a requisição de relatório RF de Compromissadas"
    ...
    def _process_webhook(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
        ...
        if not csv_data:
            self.logger.logger.info(
                "Não foram encontrados dados para o relatório RF de Compromissadas"
            )
            return {
                "message": "Não foram encontrados dados para o relatório RF de Compromissadas"
            }, HTTPStatus.NO_CONTENT

        self.logger.logger.info("Relatório RF de Compromissadas gerado com sucesso.")
