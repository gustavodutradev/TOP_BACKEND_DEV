...
from core.services.fixed_income_reports.government_bond_service import GovBondService
...
class GovBondController:
    ...
    def _handle_initial_request(self) -> Tuple[Dict[str, Any], int]:
        ...
        self.logger.log_and_respond(
            "Iniciando requisição de relatório de Títulos Públicos."
        )

        if not self.gov_bond_service.get_gov_bond_report():
            return {
                "error": "Falha ao iniciar a requisição de relatório de Títulos Públicos."
    ...
    def _process_webhook(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
        ...
        csv_data = self.gov_bond_service.process_csv_from_url(csv_url)
        if not csv_data:
            self.logger.logger.info(
                "Não foram encontrados dados para o relatório de Títulos Públicos."
            )
            return {
                "message": "Não foram encontrados dados para o relatório de Títulos Públicos."
            }, HTTPStatus.NO_CONTENT

        self.logger.logger.info("Relatório de Títulos Públicos gerado com sucesso.")
