...
class PositionsByPartnerController:
    ...
    def _handle_initial_request(self) -> Tuple[Dict[str, Any], int]:
        ...
        if not self.positions_by_partner_service.get_positions_by_partner():
            return {
    ...
    def _process_webhook(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
        ...
        csv_data = self.positions_by_partner_service.process_csv_from_url(csv_url)
        if not csv_data:
            self.logger.logger.info(
                "Não foram encontrados dados para o relatório Base BTG."
            )
            return {
                "message": "Não foram encontrados dados para o relatório Base BTG."
            }, HTTPStatus.NO_CONTENT

        self.logger.logger.info("Relatório Base BTG gerado com sucesso.")
