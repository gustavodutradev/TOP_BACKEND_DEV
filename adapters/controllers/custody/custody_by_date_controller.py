...
class CustodyByDateController:
    ...
    def register_routes(self) -> None:
        ...
        self.app.add_url_rule(
            "/api/v1/custody-by-date",
            "rg_custody_by_date_handler",
            self.handler,
            methods=["POST"],
        )
    ...
    def _handle_initial_request(self) -> Tuple[Dict[str, Any], int]:
        """
        Handle the initial Custody by Date report request.
        Returns:
            Tuple containing response data and HTTP status code
        """
        self.logger.log_and_respond(
            "Iniciando requisição de relatório de Custódia por Data."
        )

        if not self.custody_service.get_custody_by_date():
            return {
                "error": "Falha ao iniciar a requisição de relatório de Custódia por Data."
    ...
    def _process_webhook(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
        ...
        if not csv_data:
            self.logger.logger.info(
                "Não foram encontrados dados para o relatório de Custódia por Data."
            )
            return {
                "message": "Não foram encontrados dados para o relatório de Custódia por Data."
            }, HTTPStatus.NO_CONTENT

        self.logger.logger.info("Relatório de Custódia por Data gerado com sucesso.")
