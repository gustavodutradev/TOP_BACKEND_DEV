        ...
        try:
            ...
            if client_data:
                ...
                advisor_data = next(
                    (
                        data
                        for data in self.advisor_data
                        if data["advisorCgeCode"] == client_data["sgCGE"]
                    ),
                    None,
                )
                ...
                if advisor_data:
                    ...
                    logger.error(
                        f"Não foi possível encontrar os dados do assessor com o código CGE {client_data['sgCGE']}"
                    )
            else:
                logger.error(
                    f"Não foi possível encontrar os dados do cliente com o número de conta {account_number}"
                )
